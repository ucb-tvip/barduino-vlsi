...
object TestUtility {
  ...
  def writeToCSV(data: Array[Int], filepath: String): Unit = {
    val writer = new PrintWriter(new File(filepath))
    try {
      data.foreach(freq => writer.write(freq.toString + "\n"))
    } finally {
      writer.close()
    }
  }

  /* LO start/stop counter emulation */
  def lo_counter(adc_clock_freq: Double = 32000000, num_adc_clock_cycles: Int = 0, f_lo: Double = 300000000, start: Boolean = true): Long = {
    if (start) {
      0
    } else {
      val counting_period = (1/adc_clock_freq) * num_adc_clock_cycles
      val lo_period = 1/f_lo
      val counter = math.floor(counting_period/lo_period).toLong
      counter
    }
  }

  def calc_freq_offset(prev_control_word: Int, new_control_word: Int, controller_active: Boolean): Double = {
    if (!controller_active) {
      return 0
    }
    var res = 0.0
    if (prev_control_word >= 0 && prev_control_word < 32) {
      res = 1.28e3
    } else if (prev_control_word >= 32 && prev_control_word < 64) {
      res = 1.14e3
    } else if (prev_control_word >= 64 && prev_control_word < 96) {
      res = 1.00e3
    } else if (prev_control_word >= 96 && prev_control_word < 128) {
      res = 0.86e3
    } else if (prev_control_word >= 128 && prev_control_word < 160) {
      res = 0.72e3
    } else if (prev_control_word >= 160 && prev_control_word < 192) {
      res = 0.58e3
    } else if (prev_control_word >= 192 && prev_control_word < 224) {
      res = 0.44e3
    } else if (prev_control_word >= 224 && prev_control_word < 256) {
      res = 0.30e3
    }
    (new_control_word - prev_control_word) * res
  }

...
class BMCTest extends AnyFlatSpec with ChiselScalatestTester {
  ...
  it should "Pass a full BLE baseband loop" in {
    test(new BMCSingleClock(params, beatBytes)).withAnnotations(Seq(TreadleBackendAnnotation, WriteVcdAnnotation, PrintFullStackTraceAnnotation)) { c =>
      ...
      for (i <- 0 until tests) {
        ...
        fork
          .withRegion(Monitor) {
            println("HELLOOOOSUB1")
            c.io.dma.readReq.waitForValid()
            println("HELLOOOOSUB2")
            c.io.dma.readReq.valid.expect(true.B)
          }
          .joinAndStep(c.clock)
        // c.clock.step()
        println("HELLOOOOSUB3")
        println(c.io.dma.readReq.ready.peek())
        println(c.io.dma.readReq.valid.peek())
        println(c.io.dma.readReq.bits.peek())
        val pduLength = c.io.dma.readReq.bits.totalBytes.litValue.intValue
        println("HELLOOOOSUB4")

...
class FIR(cfg: FIRConfig) extends Module {
  val io = IO(new Bundle {
    ...
    val in  = Flipped(Decoupled(FixedPoint(cfg.dataType.getWidth.W, cfg.dataType.binaryPoint)))
    val out = Decoupled(FixedPoint(cfg.dataType.getWidth.W, cfg.dataType.binaryPoint))
  ...
  val zs = io.in.bits +: Seq.fill(coeffs.length - 1)(Reg(FixedPoint(cfg.dataType.getWidth.W, cfg.dataType.binaryPoint))) // zs[i] = in[n-i]

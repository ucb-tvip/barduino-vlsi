...
class ModemAnalogIO(val params: BasebandModemParams) extends Bundle {
  ...
  val tuning = new ModemTuningDataIO // problematic wires come from this
}

// Temporary fix for correct ChipTop (Andy K)
class ModemAnalogPunchIO(val params: BasebandModemParams) extends Bundle {
  val tx = new AnalogTXIO
  val rx = new AnalogRXIO(params)
...
class AnalogTXIO extends Bundle {
  val vco = new Bundle {
    // val pwr_tuning = Output(UInt(10.W))
...
class AnalogRXIO(val params: BasebandModemParams) extends Bundle {
  val i = new Bundle {
    ...
    // val mux_ctrl = Output(UInt(params.adcMuxCtrlBits.W))
  }
  val q = new Bundle {
    val data = Input(UInt(params.adcBits.W))
    val valid = Input(Bool())
...
class ModemTuningIO extends Bundle {
  val trim = new Bundle {
    ...
    val g1 = Input(UInt(8.W)) // Trim G1 is an input
  ...
  val enable = new Bundle {
    ...
    val rx1 = Output(Bool())
    val rx2 = Output(Bool())
    val vco_lo = Output(Bool())
    val ext_lo = Output(Bool())
    val pa = Output(Bool())
  }
  val mux = new Bundle {
    val dbg = new Bundle {
  ...
  val adc_clk_tuning = Output(UInt(9.W))

  val pwr_tuning = Output(UInt(10.W))

  val adc = new Bundle {
    val i = new Bundle {
      val mux_ctrl = Output(UInt(2.W))
    }
    val q = new Bundle {
      val mux_ctrl = Output(UInt(2.W))
    }
...
class Modem(params: BasebandModemParams) extends Module {
  ...
  val i_buf = RegInit(0.U(params.adcBits.W))
  val q_buf = RegInit(0.U(params.adcBits.W))
  val i_buf_buf = RegNext(i_buf)
  val q_buf_buf = RegNext(q_buf)
  when (~io.analog.rx.i.valid) {
    i_buf := io.analog.rx.i.data
  }
  when (~io.analog.rx.q.valid) {
    q_buf := io.analog.rx.q.data
  }
  ...
  rx.io.analog.i := i_buf_buf
  rx.io.analog.q := q_buf_buf
  ...
  io.adcClockTuning <> rx.io.adcClockTuning
  rx.io.lo_counter := io.lo_counter

  /* AGC */
  val iAGC = Module(new AGCWrapper(params))
  iAGC.io.control := io.control.tuning.i.AGC.control
  iAGC.io.adcIn := i_buf_buf

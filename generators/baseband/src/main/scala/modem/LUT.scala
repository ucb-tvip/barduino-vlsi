...
object ModemLUTConfigs {
  val VCO_MOD       = ModemLUTConfig(0.U, 255.U, 64, UInt(8.W))
  val VCO_CT_BLE    = ModemLUTConfig(1.U, 13248.U, 64, UInt(16.W)) // only 40 entries used
  val VCO_CT_LRWPAN = ModemLUTConfig(2.U, 13248.U, 64, UInt(16.W)) // only 16 entries used

  // only 32 entries used
  val AGC_I = ModemLUTConfig(3.U, 0.U, 32, UInt(10.W))
  val AGC_Q = ModemLUTConfig(4.U, 0.U, 32, UInt(10.W))

  // only 32 entries used
  val DCO_I = ModemLUTConfig(5.U, 0.U, 64, UInt(6.W))
  val DCO_Q = ModemLUTConfig(6.U, 0.U, 64, UInt(6.W))
}
...
class ModemLUT(cfg: ModemLUTConfig) extends Module {
  ...
  val reg = RegInit(VecInit.tabulate(cfg.len){i=>cfg.init.pad(cfg.typ.getWidth)})

...
class GFSKTX(params: BasebandModemParams) extends Module {
  ...
  // A hacky trick to remedy evaluation of the register at reset as 
  // a constant type in FIRRTL pass
  val zeroDynamic = 0.F(firWidth, firBP) + 0.F(firWidth, firBP)
  val firInData = RegInit(zeroDynamic)
  ...
  when(state === s_idle) {
    ...
    when(counter === 0.U) {
      when(io.digital.in.fire) {
        firInValid := true.B
        firInData := Mux(io.digital.in.bits === 0.U,
                 (-1).F(firWidth, firBP).asTypeOf(firInData),
                 1.F(firWidth, firBP).asTypeOf(firInData))
        counter := counter + 1.U
  ...
  io.modIndex := Mux(state === s_working, modIdx, 31.U)
}
...
class MSKTX(params: BasebandModemParams) extends Module {
  ...
  io.modIndex := Mux(state === State.s_working, outModIndex, zFd)
}
...
class GFSKTXWrapper(params: BasebandModemParams) extends Module {
  ...
  val gfskTX = Module(new GFSKTX(params))

  val digInQueue = Module(new Queue(UInt(1.W), params.modemQueueDepth))
  ...
  val controlInputQueue = Module(new Queue(new FSKTXControlInputBundle(params), params.asyncQueueDepth))
  controlInputQueue.io.enq <> io.control.in
  gfskTX.io.control <> controlInputQueue.io.deq

  val outputQueue = Module(new Queue(new FSKTXOutputIO(), params.asyncQueueDepth))
  ...
  io.analog.modIndex := gfskTX.io.modIndex
}
...
class MSKTXWrapper(params: BasebandModemParams) extends Module {
  ...
  val mskTX = Module(new MSKTX(params))
  ...
  io.control.out.done := mskTX.io.out.done
  io.state := mskTX.io.out.state
  io.analog.modIndex := mskTX.io.modIndex
}
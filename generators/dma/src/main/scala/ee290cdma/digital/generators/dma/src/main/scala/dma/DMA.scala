...
class EE290CDMAWriter(beatBytes: Int, name: String)(implicit p: Parameters) extends LazyModule {
  ...
  // )
  val node = TLClientNode(Seq(TLMasterPortParameters.v1(Seq(TLMasterParameters.v1(name = name, sourceId = IdRange(0, 1))))))
...
class EE290CDMAReader(beatBytes: Int, maxReadBytes: Int, name: String)(implicit p: Parameters) extends LazyModule {
  ...
  // )
  val node = TLClientNode(Seq(TLMasterPortParameters.v1(Seq(TLMasterParameters.v1(name = name, sourceId = IdRange(1, 2))))))
